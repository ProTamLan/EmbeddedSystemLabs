...
void SetVT(void); //high
void ClearVT(void); //low
void SetReady(void);
...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
		SetReady();	//a) Make the Ready signal go high
		WaitForASLow();	//b) Wait for AS to be 0, switch to be pressed
		ClearReady();	//c) Make the Ready signal go low
		Delay1ms(10);	//d) Wait 10ms
		WaitForASHigh();	//e) Wait for AS to be 1, switch to be released
		Delay1ms(250);
		SetVT();	//g) Make the VT signal go high
		Delay1ms(250);
		ClearVT();	//i) Make the VT signal go low
  }
}
...
void SetVT(void){
	GPIO_PORTF_DATA_R |=0x02; //set 0000 0010
}
...
// Notes:   friendly means it does not affect other bits in the port
void ClearVT(void){
	GPIO_PORTF_DATA_R &=!0x02; //clear invert 0000 0010 = 0000 0000
}
...
// Notes:   friendly means it does not affect other bits in the port
void SetReady(void){
	GPIO_PORTF_DATA_R |=0x08; // 0000 1000 = 8 = Setting (or/|) Green
}
...
// Notes:   friendly means it does not affect other bits in the port
void ClearReady(void){
	GPIO_PORTF_DATA_R &=!0x08; //clear low bit and inverting (now val 3)
}

// Subroutine to delay in units of milliseconds (0.1s)
