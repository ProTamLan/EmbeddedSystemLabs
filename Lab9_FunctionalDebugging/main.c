...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_PUR_R = 0x11;          // enable pull-up R on PF0 and PF4
...
// first data point is wrong (not comparing), the other 49 will be correct
...
int main(void){  unsigned long i,last,now; 
  ...
  while(1){
		...
		if (!SW1 || !SW2){ // negative logic both press = 1 (on)
			Led = GPIO_PORTF_DATA_R;   // read previous
			Led = Led^0x02;            // toggle red LED
			GPIO_PORTF_DATA_R = Led;   // output
			Delay();			
    if(i<50){
      now = NVIC_ST_CURRENT_R; // now = current time
      Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
      Data[i] = GPIO_PORTF_DATA_R&0x13; // record PF1 (changed Part D.)
      last = now;
      i++;
    }
    } else{ //both off, do nothing, no led
